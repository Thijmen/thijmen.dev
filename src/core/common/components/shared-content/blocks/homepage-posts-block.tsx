...
import type { MyHomepagePostsBlock, Post, Tag } from '@/payload/payload-types'
    ...
    <section className="py-2 space-y-6">
      ...
      <div className="grid pt-4 grid-cols-1 sm:grid-cols-2 gap-8">
        {cmsPosts.map((post, index) => {
          // Map the tags from the post to the TagProps format
          const mappedTags = post.tags
            ? (post.tags as Tag[]).map((tag) => ({
                slug: tag.slug || `tag-${tag.id}`, // Fallback if slug is null
                name: tag.name,
              }))
            : []
            ...
            >
              <Card href={`/posts/${post.slug}`} variant="post">
                ...
                <Card.Content>
                  ...
                  <Card.Tags variant="post" tags={mappedTags} />
