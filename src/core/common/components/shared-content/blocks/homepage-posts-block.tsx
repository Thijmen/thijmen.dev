    ...
    <section className="py-2">
      ...
      <div className="grid pt-4 grid-cols-1 lg:grid-cols-2 gap-8">
        {thePosts.map((post, index) => (
          ...
          >
            <div className="absolute inset-0 bg-gradient-to-r from-violet-500/[0.08] via-purple-500/[0.08] to-pink-500/[0.08] dark:from-violet-400/[0.03] dark:via-purple-400/[0.03] dark:to-pink-400/[0.03] rounded-2xl transform transition-all duration-500 group-hover:scale-[1.02] group-hover:opacity-100 opacity-0 blur-sm" />
            <div
              className="relative bg-gradient-to-b from-white to-gray-50 dark:from-gray-900/60 dark:to-gray-800/40 rounded-2xl border border-gray-200/70 dark:border-gray-800/40 overflow-hidden transition-all duration-500 group-hover:border-gray-300 dark:group-hover:border-gray-700/60"
              style={{
                boxShadow: `
                  ...
                  0 0 0 1px rgb(255 255 255 / 1) inset
                `,
              }}
            >
              <div className="relative p-6 space-y-4 bg-gradient-to-br from-transparent via-white/50 to-gray-50/80 dark:from-transparent dark:via-gray-900/20 dark:to-gray-800/40">
                <div className="flex flex-wrap gap-2">
                  {tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-3 py-1 text-[11px] font-medium tracking-wider uppercase bg-white dark:bg-gray-800/80 text-gray-600 dark:text-gray-300 rounded-full ring-1 ring-gray-200/70 dark:ring-white/5 shadow-[0_1px_2px_rgba(0,0,0,0.05)] dark:shadow-none backdrop-blur-sm"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <div className="flex justify-between items-center pt-4 border-t border-gray-200/50 dark:border-white/[0.04]">
                  <time className="text-xs tracking-wide text-gray-500 dark:text-gray-500">
                  ...
                  <Link
                    href={`/posts/${post.slug}`}
                    className="inline-flex items-center text-xs tracking-wide text-indigo-600 dark:text-indigo-300 font-medium group/link hover:text-indigo-700 dark:hover:text-indigo-200 transition-colors duration-300"
                  >
                    <span className="relative">
                      Read article
                      <span className="absolute inset-x-0 -bottom-0.5 h-px bg-gradient-to-r from-indigo-600/0 via-indigo-600/50 to-indigo-600/0 dark:from-indigo-300/0 dark:via-indigo-300/40 dark:to-indigo-300/0 transform origin-left scale-x-0 transition-transform duration-300 group-hover/link:scale-x-100" />
                    </span>
                    <svg
